...
"#,
    ...
    assert!(candidates.contains("after-global-option:help login logout skill whoami workspace config alias project client tag task entry today timeline status split switch timer completion"));
