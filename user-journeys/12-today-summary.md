...
6. Run `cfd today --workspace <confirmed-workspace-id> --sort desc`
7. Run `cfd today --workspace <confirmed-workspace-id> --format json`
8. Run `cfd entry list --workspace <confirmed-workspace-id> --start today --end today --format json`
9. Compare the JSON output from steps 7 and 8 semantically
10. If there is no entry today and the user approved a temporary entry, create one:
...
11. Run `cfd today --workspace <confirmed-workspace-id>` again
12. If the user approves testing a running entry and no timer is running, run:
    `cfd timer start "cfd today running journey" --workspace <confirmed-workspace-id> --project <confirmed-project-id>`
13. Run `cfd today --workspace <confirmed-workspace-id>` while the timer is running
14. Stop the timer if it was created by this journey:
    `cfd timer stop --workspace <confirmed-workspace-id> -y`
15. Capture returned entry IDs from any temporary entries
...
- Default `cfd today` output sorts by start time ascending, so the newest entry is at the bottom
- `cfd today --sort desc` shows newest entries first
